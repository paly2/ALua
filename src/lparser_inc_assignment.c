...
static void assignment (LexState *ls, struct LHS_assign *lh, int nvars) {
  ...
  if (testnext(ls, ',')) {  /* assignment -> ',' suffixedexp assignment */
    ...
    if(nvars==1) {
      switch(ls->t.token) {
        case TK_SCPLUS: case TK_SCMINUS: case TK_SCONCAT:
...
static void shortcut_op (LexState *ls, expdesc *v) {
  #if 0
  expdesc e1, e2;
  BinOpr op = OPR_NOBINOPR;
  //int reg=ls->fs->freereg;
  ...
  luaX_next(ls);
  expr(ls, &e2);
  luaK_posfix(ls->fs, op, &e1, &e2, ls->linenumber);
  leavelevel(ls);
